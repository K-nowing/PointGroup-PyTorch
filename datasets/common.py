...
def grid_subsampling(points, features=None, semantic_labels=None, instance_labels=None, sampleDl=0.1, verbose=0):
    """
    ...
    if (features is None) and (semantic_labels is None) or (instance_labels is None):
        return cpp_subsampling.subsample(points,
                                         sampleDl=sampleDl,
                                         verbose=verbose)
    elif (semantic_labels is None) or (instance_labels is None):
        return cpp_subsampling.subsample(points,
                                         features=features,
                                         sampleDl=sampleDl,
                                         verbose=verbose)
        ...
        return cpp_subsampling.subsample(points,
                                         ...
                                         instance_classes=instance_labels,
                                         sampleDl=sampleDl,
                                         verbose=verbose)
    else:
        return cpp_subsampling.subsample(points,
...
def batch_grid_subsampling(points, batches_len, features=None, semantic_labels=None, instance_labels=None,
    ...
    if (features is None) and (semantic_labels is None) or (instance_labels is None):
        ...
        s_points, s_len, s_semantic_labels, s_instance_labels = cpp_subsampling.subsample_batch(points,
                                                                    batches_len,
                                                                    semantic_classes=semantic_labels,
                                                                    instance_classes=instance_labels,
                                                                    sampleDl=sampleDl,
        ...
        if random_grid_orient:
            ...
            for bi, length in enumerate(s_len):
                ...
                s_points[i0:i0 + length, :] = np.sum(np.expand_dims(s_points[i0:i0 + length, :], 2) * R[bi].T, axis=1)
                i0 += length
        return s_points, s_len, s_semantic_labels, s_instance_labels

    else:
        s_points, s_len, s_features, s_semantic_labels, s_instance_labels = cpp_subsampling.subsample_batch(points,
                                                                              batches_len,
                                                                              features=features,
                                                                              semantic_classes=semantic_labels,
                                                                              instance_classes=instance_labels,
        ...
        if random_grid_orient:
            ...
            for bi, length in enumerate(s_len):
                ...
                s_points[i0:i0 + length, :] = np.sum(np.expand_dims(s_points[i0:i0 + length, :], 2) * R[bi].T, axis=1)
                i0 += length
        return s_points, s_len, s_features, s_semantic_labels, s_instance_labels
...
class PointCloudDataset(Dataset):
    ...
    def segmentation_inputs(self,
        ...
        li += [stacked_features, semantic_labels, instance_labels, instance_centers, cloud_slices]
