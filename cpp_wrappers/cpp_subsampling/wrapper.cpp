...
static PyObject* batch_subsampling(PyObject* self, PyObject* args, PyObject* keywds)
{
	...
	PyObject* semantic_classes_obj = NULL;
	PyObject* instance_classes_obj = NULL;
	PyObject* batches_obj = NULL;

	// Keywords containers
	static char* kwlist[] = { "points", "batches", "features", "semantic_classes", "instance_classes", "sampleDl", "method", "max_p", "verbose", NULL };
	float sampleDl = 0.1;
	...
	// Parse the input  
	if (!PyArg_ParseTupleAndKeywords(args, keywds, "OO|$OOOfsii", kwlist, &points_obj, &batches_obj, &features_obj, &semantic_classes_obj, &instance_classes_obj, &sampleDl, &method_buffer, &max_p, &verbose))
	{
	...
	if (features_obj == NULL)
		use_feature = false;
	if (semantic_classes_obj == NULL && instance_classes_obj == NULL)
		use_classes = false;
	...
	PyObject* semantic_classes_array = NULL;
	PyObject* instance_classes_array = NULL;
	if (use_feature)
		features_array = PyArray_FROM_OTF(features_obj, NPY_FLOAT, NPY_IN_ARRAY);
	if (use_classes)
	{
		semantic_classes_array = PyArray_FROM_OTF(semantic_classes_obj, NPY_INT, NPY_IN_ARRAY);
		instance_classes_array = PyArray_FROM_OTF(instance_classes_obj, NPY_INT, NPY_IN_ARRAY);
	}
	...
	if (points_array == NULL)
	{
		...
		Py_XDECREF(semantic_classes_array);
		Py_XDECREF(instance_classes_array);
	...
	if (use_classes && semantic_classes_array == NULL && instance_classes_array == NULL)
	{
		Py_XDECREF(points_array);
		Py_XDECREF(batches_array);
		Py_XDECREF(semantic_classes_array);
	...
	if (use_classes && (int)PyArray_NDIM(semantic_classes_array) > 2 && (int)PyArray_NDIM(instance_classes_array) > 2)
	{
		Py_XDECREF(points_array);
		Py_XDECREF(batches_array);
		Py_XDECREF(semantic_classes_array);
	...
	int ldim = 1;
	if (use_classes && (int)PyArray_NDIM(semantic_classes_array) == 2 && (int)PyArray_NDIM(instance_classes_array) == 2)
		ldim = (int)PyArray_DIM(semantic_classes_array, 1);
		// ldim = (int)PyArray_DIM(instance_classes_array, 1);
	...
	if (use_classes && (int)PyArray_DIM(semantic_classes_array, 0) != N && (int)PyArray_DIM(instance_classes_array, 0) != N)
	{
		Py_XDECREF(points_array);
		Py_XDECREF(batches_array);
		Py_XDECREF(semantic_classes_array);
	...
	if (use_classes)
	{
		original_semantic_classes = vector<int>((int*)PyArray_DATA(semantic_classes_array), (int*)PyArray_DATA(semantic_classes_array) + N * ldim);
		original_instance_classes = vector<int>((int*)PyArray_DATA(instance_classes_array), (int*)PyArray_DATA(instance_classes_array) + N * ldim);
	}
	// Subsample
	...
	batch_grid_subsampling(original_points,
							...
							original_features,
							subsampled_features,
							original_semantic_classes,
							original_instance_classes,
							subsampled_semantic_classes,
							subsampled_instance_classes,
							original_batches,
							subsampled_batches,
							sampleDl,
							max_p);
	...
	PyObject* res_semantic_classes_obj = NULL;
	PyObject* res_instance_classes_obj = NULL;
	PyObject* ret = NULL;
	...
	if (use_classes)
	{
		...
		res_semantic_classes_obj = PyArray_SimpleNew(2, classes_dims, NPY_INT);
		res_instance_classes_obj = PyArray_SimpleNew(2, classes_dims, NPY_INT);
		memcpy(PyArray_DATA(res_semantic_classes_obj), subsampled_semantic_classes.data(), size_in_bytes);
		memcpy(PyArray_DATA(res_instance_classes_obj), subsampled_instance_classes.data(), size_in_bytes);
	}


	// Merge results
	if (use_feature && use_classes)
		ret = Py_BuildValue("NNNNN", res_points_obj, res_batches_obj, res_features_obj, res_semantic_classes_obj, res_instance_classes_obj);
	else if (use_feature)
		ret = Py_BuildValue("NNN", res_points_obj, res_batches_obj, res_features_obj);
	else if (use_classes)
		ret = Py_BuildValue("NNNN", res_points_obj, res_batches_obj, res_semantic_classes_obj, res_instance_classes_obj);
	else
...
static PyObject* cloud_subsampling(PyObject* self, PyObject* args, PyObject* keywds)
{
	...
	// Keywords containers
	static char* kwlist[] = { "points", "features", "semantic_classes", "instance_classes", "sampleDl", "method", "verbose", NULL };
	float sampleDl = 0.1;
	const char* method_buffer = "barycenters";
	int verbose = 0;
	
	// Parse the input  
	if (!PyArg_ParseTupleAndKeywords(args, keywds, "O|$OOOfsi", kwlist, &points_obj, &features_obj, &semantic_classes_obj, &instance_classes_obj, &sampleDl, &method_buffer, &verbose))
	{
		PyErr_SetString(PyExc_RuntimeError, "Error parsing arguments");
		return NULL;
	}
	...
	// Merge results
	if (use_feature && use_classes)
		ret = Py_BuildValue("NNNN", res_points_obj, res_features_obj, res_semantic_classes_obj, res_instance_classes_obj);
	else if (use_feature)
		ret = Py_BuildValue("NN", res_points_obj, res_features_obj);
	else if (use_classes)
		ret = Py_BuildValue("NNN", res_points_obj, res_semantic_classes_obj, res_instance_classes_obj);
	else
