...
void grid_subsampling(vector<PointXYZ>& original_points,
	...
	size_t ldim = original_semantic_classes.size() / N;
	// size_t ldim = original_instance_classes.size() / N;
	...
	bool use_classes = original_semantic_classes.size() > 0;
	// bool use_instance_classes = original_instance_classes.size() > 0;
	...
	for (auto& p : original_points)
	{
		...
		if (use_feature && use_classes)
			data[mapIdx].update_all(p, original_features.begin() + i * fdim, original_semantic_classes.begin() + i * ldim, original_instance_classes.begin() + i * ldim);
		else if (use_feature)
			data[mapIdx].update_features(p, original_features.begin() + i * fdim);
		else if (use_classes)
			data[mapIdx].update_classes(p, original_semantic_classes.begin() + i * ldim, original_instance_classes.begin() + i * ldim);
		else
	...
	if (use_classes)
	{
		subsampled_semantic_classes.reserve(data.size() * ldim);
		subsampled_instance_classes.reserve(data.size() * ldim);
	}
	for (auto& v : data)
	{
		...
		if (use_classes)
		{
			...
			{
		        subsampled_semantic_classes.push_back(max_element(v.second.semantic_labels[i].begin(), v.second.semantic_labels[i].end(),
		        [](const pair<int, int>&a, const pair<int, int>&b){return a.second < b.second;})->first);
				subsampled_instance_classes.push_back(max_element(v.second.instance_labels[i].begin(), v.second.instance_labels[i].end(),
		        [](const pair<int, int>&a, const pair<int, int>&b){return a.second < b.second;})->first);
			}
		}
	}
                              ...
                              int max_p)
{
	...
	for (b = 0; b < original_batches.size(); b++)
	{
	    ...
	    vector<int> b_o_semantic_classes;
		vector<int> b_o_instance_classes;
        if (original_semantic_classes.size() > 0 && original_instance_classes.size() > 0)
        {
            b_o_semantic_classes = vector<int>(original_semantic_classes.begin () + sum_b * ldim,
                                      original_semantic_classes.begin () + sum_b + original_batches[b] * ldim);
			b_o_instance_classes = vector<int>(original_instance_classes.begin () + sum_b * ldim,
                                      original_instance_classes.begin () + sum_b + original_batches[b] * ldim);
	    }
        ...
        vector<int> b_s_semantic_classes;
		vector<int> b_s_instance_classes;
        ...
        grid_subsampling(b_o_points,
                         ...
                         b_o_features,
                         b_s_features,
                         b_o_semantic_classes,
						 b_o_instance_classes,
						 b_s_semantic_classes,
						 b_s_instance_classes,
                         sampleDl,
						 0);
        ...
        if (b_s_points.size() <= max_p)
        {
			...
			if (original_semantic_classes.size() > 0)
                subsampled_semantic_classes.insert(subsampled_semantic_classes.end(), b_s_semantic_classes.begin(), b_s_semantic_classes.end());

            if (original_instance_classes.size() > 0)
                subsampled_instance_classes.insert(subsampled_instance_classes.end(), b_s_instance_classes.begin(), b_s_instance_classes.end());
        ...
        else
        {
            ...
            if (original_semantic_classes.size() > 0)
                subsampled_semantic_classes.insert(subsampled_semantic_classes.end(), b_s_semantic_classes.begin(), b_s_semantic_classes.begin() + max_p * ldim);

			if (original_instance_classes.size() > 0)
                subsampled_instance_classes.insert(subsampled_instance_classes.end(), b_s_instance_classes.begin(), b_s_instance_classes.begin() + max_p * ldim);
